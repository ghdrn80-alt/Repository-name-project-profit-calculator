...
import { SetupCost } from '../types'
import NumberInput from './NumberInput'

interface Props {
  data: SetupCost[]
  onChange: (data: SetupCost[]) => void
}

function SetupCostForm({ data, onChange }: Props) {
  const lastAddedRef = useRef<string | null>(null)
  const tableRef = useRef<HTMLTableElement>(null)

  const addItem = () => {
    const newItem: SetupCost = {
  ...
  const updateItem = (id: string, field: keyof SetupCost, value: string | number) => {
    onChange(
      data.map((item) =>
  ...
  return (
    <section className="form-section">
      ...
      {data.length === 0 ? (
        <p className="empty-message">시운전/셋업 항목을 추가하세요</p>
      ) : (
        <table className="data-table" ref={tableRef}>
          ...
          <tfoot>
            <tr>
              <td colSpan={3} className="total-label">시운전/셋업 합계</td>
